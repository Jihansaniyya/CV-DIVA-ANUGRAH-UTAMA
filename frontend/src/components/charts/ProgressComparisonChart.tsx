    ...
    <div style={{ width: '100%', height: 280 }}>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={titik} margin={{ top: 8, right: 8, bottom: 4, left: -8 }}>
          ...
          />
          <Tooltip
            formatter={(nilai, nama) => [`${angka(Number(nilai), 2)}%`, nama]}
            labelFormatter={(_, isi) => String(isi?.[0]?.payload?.nama_proyek ?? '')}
            cursor={{ fill: '#f5f7fa' }}
            contentStyle={{ borderRadius: 8, border: '1px solid #e2e8f0', fontSize: 12, maxWidth: 320, whiteSpace: 'normal' }}
          />
